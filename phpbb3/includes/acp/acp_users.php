...
/*
// begin hack
$sqlSelect = 'select user_id, username from '. USERS_TABLE;

$res = $db->sql_query($sqlSelect);

$usersArr = array();
while($userRow = $db->sql_fetchrow($res))
{
	 $usersArr[] = array(0 => $userRow['user_id'], 1 => addslashes(utf8_clean_string($userRow['username'])));
}

foreach ( $usersArr as $key=>$value)
{
	$sqlUpdate = "UPDATE ".USERS_TABLE."  SET username_clean = '".$value[1]."' where user_id=".$value[0];
...
//echo "<br>".$sql."<br>".utf8_clean_string($username);
