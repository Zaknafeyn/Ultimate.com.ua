...
if (!check_access('manual'))
{
	...
	function DelChild($id) {
        $rst = $db->sql_query("SELECT * FROM manual WHERE parent_id=$id");
        while ($line = $db->sql_fetchrow($rst))
            DelChild($line['id']);
        $db->sql_query("DELETE FROM manual WHERE id=$id");
    }
...
if($action=="delete" && $_REQUEST['id']!="")
{
	...
	if($_REQUEST['id']) {		$sql = "UPDATE manual
		...
		$db->sql_query($sql);
	} elseif ($_REQUEST['parent_id']) {
        $rst = $db->sql_query("SELECT MAX(o) as maxo FROM manual WHERE parent_id=".$_REQUEST['parent_id']);
    ...
    if ($action=="moveup")
    	$sql = "SELECT MAX(o) as o FROM manual WHERE parent_id=".$_REQUEST['parent_id']." AND o<".$_REQUEST['o'];
    else
    	$sql = "SELECT MIN(o) as o FROM manual WHERE parent_id=".$_REQUEST['parent_id']." AND o>".$_REQUEST['o'];
	$rst = $db->sql_query($sql);
    if ($db->sql_affectedrows($rst)) {
    	$line = $db->sql_fetchrow($rst);
        $db->sql_query("UPDATE manual SET o=".$_REQUEST['o']." WHERE parent_id=".$_REQUEST['parent_id']." AND o=".$line['o']);
        $db->sql_query("UPDATE manual SET o=".$line['o']." WHERE id=".$_REQUEST['id']);
    }
    header("location:manual.php");
    exit;
}
...
if($_REQUEST['id']!='')
{
	$art_res=$db->sql_query("select * from manual where id=".$_REQUEST['id']);
	$news=$db->sql_fetchrow($art_res);
}else{
	$art_res=$db->sql_query("select * from manual where 1=0");
	$news=$db->sql_fetchrow($art_res);
}
...
<form action="manual_det.php" method="post">
...
<tr valign="top">
	...
	<p>
		Ссылка
		<br /><a>http://ultimate.com.ua/man/.../</a><input name="char_id" style="width:200px; color: #0097ee;" value="<?=$news['char_id']?>" class="inp">/
	</p>
	<p>
		Название в меню
