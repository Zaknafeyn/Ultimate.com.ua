    <div id="topnav">
    	<? $str_to_insert = ($user->data['user_id'] != ANONYMOUS)? "" : " href=\"/$forum_folder_name/ucp.php?mode=login\"" ?>
      <span><a id="login_link" <?=$str_to_insert?> class="pseudo_link clickable"><?=($user->data['user_id'] == ANONYMOUS)?"Вход":$user->data['username_clean']?></a></span>
<!--
<span><a id="login_link"<?=$user->data['session_logged_in']?"":" href=\"/$forum_folder_name/ucp.php?mode=login\""?> class="pseudo_link clickable"><?=(!$user->data['session_logged_in'])?"Вход":$user->data['username']?></a></span>
	-->
	...
	?>
		<form method="post" id="login" action="/<?echo $forum_folder_name; ?>/ucp.php?mode=login">
				<label for="username">Имя пользователя</label><br />
				<input type="text" id="header_username" name="username" value="" title="Имя пользователя форума"  tabindex="4"/>
			<br />
				<label for="password">Пароль</label><br />
				<input type="password" id="header_password" name="password" value="" title="Пароль. Сюда пароль." tabindex="5" />
			<p>
			...
			<p>
				<div class="small">
					<a href="/<? echo $forum_folder_name; ?>/ucp.php?mode=register">Регистрация</a>
				</div>
			</p>
		</form>
	<?
    		...
    		<tr valign="top">
    			...
    			<td>
    				...
    				<?
						...
						print "<br /><a href=\"/".$forum_folder_name."/ucp.php?mode=logout&sid=".$user->data['session_id']."&redirect=..".$_SERVER['REQUEST_URI']."\">Выход</a>";
    				?>
    				</div>
    			</td>
     		</tr>
