...
function TabSwitch(oPtr){

	if (oPtr) {
		this.oPtr = oPtr;
		this.aTabs = Common.Dom.getElementsByClassName(this.oPtr, 'tab', 'div');
		this.oTabSelected = Common.Dom.getElementsByClassName(this.oPtr, 'selected', 'div')[0];

		// инициализируем переключалку
		this.oSwitch = Common.Dom.getElementsByClassName(this.oPtr, 'switch', 'ul')[0];
		this.aSwitchItems = Common.Dom.getElementsByClassName(this.oSwitch, 'item', 'li');
		this.oSwitchItemSelected = Common.Dom.getElementsByClassName(this.oSwitch, 'selected', 'li')[0];
...
TabSwitch.prototype.changeImage = function(oSwitchItem){

	Common.Class.remove(this.oSwitchItemSelected, 'selected');
	Common.Class.add(oSwitchItem, 'selected');
	this.oSwitchItemSelected = oSwitchItem;

	var aClassName = oSwitchItem.className.match(new RegExp('for_(\\w+)'))[1];
	var oTab = Common.Dom.getElementsByClassName(this.oPtr, aClassName, 'div')[0];
	Common.Class.remove(this.oTabSelected, 'selected');
	Common.Class.add(oTab, 'selected');
	this.oTabSelected = oTab;
}