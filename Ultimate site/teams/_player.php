<?
	include_once $_SERVER['DOCUMENT_ROOT']."/teams/admin_player.php";
    ...
    if (isset($_REQUEST['pid'])) {
    	...
    	if ($db->sql_affectedrows($rst)) {
			...
			$meta_description = $fullname.($team_name?($cap?", капитан":", игрок") . " команды " . $team_name:"")/* . " ". stripslashes(mycut($line['Nick']))*/;
    ...
    if ($pid) {
	        ...
	        $updated = false;
	        if ($edit_mode && isset($_POST['name']) && ($user->data['user_id'] != ANONYMOUS)) {
	            ...
	            if ($db->sql_affectedrows($rst)) {
            		...
            		if ($rights['all_rights'] || $rights['team_'.$line['id_team'].'_edit'] || $userdata['user_id']==$pid) {
	                            ...
	                            if (preg_match('#image\/[x\-]*([a-z]+)#', $_FILES["fname"]["type"], $filetype)) {
	                                //if ($_FILES["fname"]["size"] <= $max_upload_file_size) {
                                        ...
                                        if (move_uploaded_file($fname,"photo/$m_photo")) {
						                    ...
						                    //imageresize($_SERVER['DOCUMENT_ROOT']."/teams/photo/$m_photo_small", $_SERVER['DOCUMENT_ROOT']."/teams/photo/$m_photo", 75, 100, 85);
	                    ...
	                    if (isset($_POST['show_twitter_post'])) { $show_twitter_post = 1; } else { $show_twitter_post = 0; }
	                    ...
	                    if (isset($_POST['city111']))     { $city = mysql_escape_string($_POST['city111']); } else { $city = '';}
	                    if (isset($_POST['country']))     { $country = mysql_escape_string($_POST['country']); } else { $country = ''; }

	                    if (isset($_POST['c_vkontakte']))	{ $c_vkontakte = preg_replace("/[^0-9a-zA-Z_-]/i","",$_POST['c_vkontakte']); } else { $c_vkontakte = ''; }
	                    ...
	                    if (isset($_POST['c_facebook']))		{ $c_facebook = preg_replace("/[^0-9a-z\.\?\&=]/i","",$_POST['c_facebook']); } else { $c_facebook = ''; }
	                    ...
	                    if (isset($_POST['sex'])) { $sex = mysql_escape_string($_POST['sex']); } else { $sex = 1; }
	                        ...
	                        $sql = "UPDATE players SET
	                            ...
	                            email='$email', icq='$icq', site='$site', skype='$skype', twitter='$twitter', show_twitter_post=$show_twitter_post, about='$about',
							...
							//турниры
	                    $r = $db->sql_query("SELECT * FROM tourn WHERE dat_begin<'".date("Ymd")."' AND dat_begin>'".$year_started_playing."0100' AND players_can_set_results=1");
	                    $tourn = array();
	                    while ($l = $db->sql_fetchrow($r))
	                    	if (isset($_REQUEST["tourn".$l['id']]))
	                    		$tourn[$l['id']] = 1;
	                    	else
	                    		$tourn[$l['id']] = 0;

						$db->sql_query("DELETE FROM players_tourn WHERE id_player=$pid");
	                  	foreach($tourn as $k=>$v)
	                  		$db->sql_query("INSERT INTO players_tourn SET id_player=$pid, id_tourn=$k, ptcp=$v");
						...
						foreach($player1 as $k=>$v) {
							...
							$mail_subj = "ultimate.com.ua:".NameSurname($pid).($pid==$userdata['user_id']?"":" (".$userdata['user_id'].")");
...
?>
	...
	<style>
		...
		.info_par		{ padding-top: 5; padding-right: 10; font-size: 12px; color: #AAAAAA; }
	    ...
	    .pltourn        { color: #444444; font-size: 12px; }
            		...
            		<?
            			if (!$edit_mode) {
	            			...
	            			if (user_has_rights_to_set_tourn_participation($pid)) {
	            			//if ($userdata['user_id']==$pid || $rights['all_rights']) {
		            			if ($db->sql_affectedrows($db->sql_query("SELECT * FROM players_tourn WHERE id_player=$pid AND ptcp=1"))) {
        		    				// отмечал(а) участие в турнирах
        		    				$r = $db->sql_query("SELECT year_started_playing FROM players WHERE id=".$pid);
        		    				$l = $db->sql_fetchrow($r);
        		    				$year_started_playing = $l['year_started_playing'];
        		    				$t = array();
                			      	$r = $db->sql_query("SELECT *, YEAR(dat_begin) AS y, MONTH(dat_begin) AS m, DAYOFMONTH(dat_begin) AS d
						    			FROM tourn
										WHERE players_can_set_results=1
										ORDER BY y DESC, m DESC, d DESC
										");
									if ($db->sql_affectedrows($r)) while ($l = $db->sql_fetchrow($r)) {
										if ( ((int)$l['dat_begin']<(int)date("Ymd")) && ((int)$l['dat_begin']>(int)($year_started_playing*10000+100)) )
											$t[$l['id']] = $l;
									}

									$r = $db->sql_query("SELECT * FROM players_tourn WHERE id_player=$pid");
									if ($db->sql_affectedrows($r)) while ($l = $db->sql_fetchrow($r)) unset($t[$l['id_tourn']]);

									if (!Sex($pid)) $sex="а";
									if (sizeof($t)) {
										print "<div id=\"ptcp\" class=\"info\">";
										print <<<EOD
<script language="javascript">
function set_tourn(id_tourn, id_user, ptcp) {
	var i=0;
	$('#preloader').html('<img src="/img/ajax-loader.gif" alt="wait for 2 hours" style="vertical-align: middle;" />');
	$.post("/teams/set_player_tourn.php", { id_tourn: id_tourn, id_user: id_user, ptcp: ptcp },
		function(data) {
			if (data!='') {
				$('#player_tournaments').html(data);
				$('#l'+id_tourn).remove();
				$('#ptcp>ul>li').each(function() { i=i+1; } );
				if (i==0) $('#ptcp').remove();
			} else {
...
EOD;
										print $userdata['username'].", участвовал$sex ли ".(($pid==$userdata['user_id']) ? "ты" : $title )." в&nbsp;".(sizeof($t)>1?"этих турнирах":"этом турнире")."?&nbsp;<span id=\"preloader\"></span><p><ul>";
										foreach ($t as $t0) {
											print "<li id=\"l".$t0['id']."\"><a target=\"_blank\" href=\"/tourn/".$t0['char_id']."/\"><img src=\"/img/new-window-icon.gif\" style=\"border: none; vertical-align: middle;\" width=\"11\" height=\"11\" alt=\"Открыть в новом окне\" /></a>";
											print "&nbsp;<b>".stripslashes($t0['short_name'])."</b>";
											print " &nbsp; &nbsp; <a class=\"pseudo_link clickable\" onclick=\"set_tourn(".$t0['id'].",".$pid.",1);return false;\">участвовал$sex</a>";
											print " &nbsp; &nbsp; <a class=\"pseudo_link clickable\" onclick=\"set_tourn(".$t0['id'].",".$pid.",0);return false;\">не&nbsp;участвовал$sex</a>";
											print "</li>";
										}
										print "</ul></div>";
									}
    	    ...
    	    <td>
                                            ...
                                            <?php
                                                    ...
                                                    if ($db->sql_affectedrows($rst)) {
                                                        ...
                                                        $forum_user = stripslashes("<a href=\"/4room/profile.php?mode=viewprofile&u=".$line['id']."\">".$line['username']."</a>");
                                                        ...
                                                        $m_show_twitter_post = $line['show_twitter_post'];
														if ($m_twitter && $m_show_twitter_post) {
															$r = $db->sql_query("SELECT descr FROM rssnews WHERE domain='http://twitter.com' AND title='$m_twitter'");
															if ($db->sql_affectedrows($r)) {
																$l = $db->sql_fetchrow($r);
                                                        ...
                                                        if ($m_is_ukr) {
                                                        	print ($cap ? "Капитан" : "Игрок"). ($tid ? " команды <span class=\"division$division\"><a href=\"/teams/$char_id/players/\">$team_name</a></span>" : ($team_name?" команды $team_name":""));
                                                        } else {
                                                        	if ($team_name)
										                    ...
										                    for ($i=0; $i<min(3,$size); $i++) {
										                        ...
										                        foreach ($up as $key=>$value) {
										                            if ($j==$k) {
																    	...
																    	if ($value['picasa_user']) {
																	    	print "<a href=\"$url\"><img style=\"border: none;\" src=\"".str_replace("/s144/","/s72/",$value['tn_fname'])."\" /></a> &nbsp; ";
																    	} else {
									                    ...
									                    if ( (($user->data['user_id'] != ANONYMOUS) && $m_id == $userdata['user_id']) || $rights['all_rights'] || $rights['team_'.$line['id_team'].'_edit'] )
									                    {
                                                        	print "<p><div class=\"small\">[ <a href=\"/players/".($m_p_char_id?$m_p_char_id:$m_id)."/edit\">Редактировать</a> ]</div></p>";
                                                        }
                                                        print "</td>";
                                                        
                                                        print "<td><div style=\"max-width:380px;min-width:300px;\"><table cellspacing=\"0\" cellpadding=\"0\" border=\"0\" width=\"100%\">";
		                                                ...
		                                                //if ($m_homeaddress||$m_workaddress||$m_homephone||$m_cellphone||$m_workphone||$m_email||$m_icq||$m_skype||$m_site) {
	                                                        ...
	                                                        if ($m_c_vkontakte || $m_c_lj || $m_c_moikrug || $m_c_odnokl || $m_c_facebook || $m_twitter) {
	                                                        	print "<tr><td colspan=\"2\">";
	                                                        	print ($m_c_vkontakte) ? "<a href=\"http://vkontakte.ru/".$m_c_vkontakte."\"><span class=\"info_par\"><img src=\"/img/c_vkontakte.gif\" height=\"22\" width=\"23\" vspace=\"10\" style=\"border: none;\" alt=\"Я ВКонтакте.ру\" /></a></span>&nbsp;&nbsp;&nbsp;" : "";
		                                                ...
		                                                if ($m_height||$m_weight||$m_tshirtsize) {
	                                                  	...
	                                                  	print "<tr><td colspan=\"2\"><div id=\"player_tournaments\">";
	                                                 	print get_player_tournaments($pid);
	                                                 	print "</div></td></tr>";

                                                        print "</table></div></td></tr></table></td></tr>";
			                                            print "</table>";
                                            			print "<table cellspacing=\"0\" cellpadding=\"15\" border=\"0\">";
                            			...
                            			<table cellspacing="0" cellpadding="15" border="0">
				    						<?
			                                    ...
			                                    if ($m_is_ukr) {
				                                    ...
				                                    $rst = $db->sql_query("SELECT * FROM teams WHERE team_active=1");
			                                    	...
			                                    	print "<p>Номер&nbsp;на&nbsp;поле:<br /><input name=\"number\" style=\"width: 350px;\" value=\"$m_number\"><br />";
				                                    print "<p>Город:<br /><input name=\"city111\" style=\"width: 350px;\" value=\"$m_city\"><br />";
			                                    ...
			                                    print "<p>Родной&nbsp;город:<br /><input name=\"city111\" style=\"width: 350px;\" value=\"$m_city\"><br />";
			                                    ...
			                                    print "<tr valign=\"top\"><td align=\"right\"><p><img src=\"/img/c_facebook.png\" style=\"vertical-align: middle;\" /> http://facebook.com/</td><td><p><input name=\"c_facebook\" style=\"width:100px;\" value=\"$m_c_facebook\" />/</td></tr>";
			                                    print "<tr valign=\"top\"><td align=\"right\"><p><img src=\"/img/twitter.png\" style=\"vertical-align: middle;\" /> http://twitter.com/</td><td><p><input name=\"twitter\" style=\"width:100px;\" value=\"$m_twitter\" />";
			                                    print "<br /><input type=\"checkbox\" id=\"show_twitter_post\" name=\"show_twitter_post\"".($m_show_twitter_post?" checked":"")." /> <label for=\"show_twitter_post\">показывать последний твит</label></td></tr>";
			                                    print "<tr valign=\"top\"><td align=\"right\"><p><img src=\"/img/c_vkontakte.gif\" style=\"vertical-align: middle;\" /> http://vkontakte.ru/</td><td><p><input name=\"c_vkontakte\" style=\"width:100px;\" value=\"$m_c_vkontakte\" /></td></tr>";
			                                ...
			                                ?>
						                        ...
						                        <td style="padding-left: 30;">
						                        	...
						                        	<?
						                        		$t0 = array();
	                                                  	$r = $db->sql_query("SELECT * FROM players_tourn WHERE id_player=$pid AND ptcp=1");
						                        		if ($db->sql_affectedrows($r)) while ($l = $db->sql_fetchrow($r))
						                        			$t0[$l['id_tourn']] = 1;
						                        		...
						                        		$r = $db->sql_query("SELECT *, YEAR(dat_begin) AS y, MONTH(dat_begin) AS m, DAYOFMONTH(dat_begin) AS d
						                        			FROM tourn
						                        			WHERE dat_begin<'".date("Ymd")."' AND dat_begin>'".$m_year_started_playing."0101' AND players_can_set_results=1
						                        			ORDER BY y DESC, m DESC, d DESC");
						                        		if ($db->sql_affectedrows($r)) while ($l = $db->sql_fetchrow($r)) {
				                                        	if ($y != $l['y']) {
				                                        		$y = $l['y'];
				                                            ...
				                                            print "<input type=\"checkbox\" id=\"tourn".$l['id']."\" name=\"tourn".$l['id']."\" style=\"border: none; width: 20px;\" value=\"".$l['id']."\"";
				                                            if (isset($t0[$l['id']]))
				                                            	print " checked";
				                                            print ">&nbsp;<label for=\"tourn".$l['id']."\"><span class=\"pltourn\">".preg_replace("/ /","&nbsp;",stripslashes($l['short_name']))." <a target=\"_blank\" href=\"/tourn/".$l['char_id']."/\"><img src=\"/img/new-window-icon.gif\" style=\"border: none;\" width=\"11\" height=\"11\" alt=\"Открыть в новом окне\" /></a></span></label><br />";
						                        		}
				                                    ?>
						                        </td>
						                        <? } ?>
